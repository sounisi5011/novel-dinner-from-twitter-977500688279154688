...
function urlFetch(
  ...
  const cacheDirRelativePath = currentPath
    ? path.relative(currentPath, cacheDirPath)
    : cacheDirPath;
  const cacheFileRelativePath = currentPath
    ...
    fs.stat(cacheFilePath, (statErr, stats) => {
      if (statErr) {
        ...
        if (statErrCode === 'ENOTDIR') {
           ...
           */
          consoleFunc(
            `The cache directory exists as a file: ${cacheDirRelativePath}`,
          );
          reject(
            new CacheDirExistsAsAFileError(
           ...
           */
          consoleFunc(
            `Failed to check if cache path exists: ${cacheFileRelativePath}`,
          );
          reject(
            new CachePathIsExistsError(
      ...
      rp({ uri: url, encoding: null })
        .then(bodyData => {
          ...
          mkdirp(cacheDirPath, mkdirErr => {
            if (mkdirErr) {
              consoleFunc(
                `Creation of cache directory failed: ${cacheDirRelativePath}`,
              );
              reject(
                new CacheDirectoryCreationError(
              ...
              fs.writeFile(cacheFilePath, bodyData, writeErr => {
                if (writeErr) {
                  consoleFunc(
                    `Creation of cache file failed: ${cacheFileRelativePath}`,
                  );
                  reject(
                    new CacheFileCreationError(
        ...
        .catch(requestErr => {
          reject(
            new HttpRequestError(`HTTP error: ${url}`, {
              code:
                requestErr.code || (requestErr.error && requestErr.error.code),
